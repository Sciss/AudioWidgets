...
import javax.swing.{Icon, JComponent, JButton, AbstractButton, BoxLayout, Box}
...
object TransportIcon {
      sealed trait ColorScheme {
         ...
         private[TransportIcon] def fillPaint( scale: Float ): Paint
      }
      case object LightScheme extends ColorScheme {
         private[TransportIcon] def fillPaint( scale: Float ): Paint = new LinearGradientPaint( 0f, 0f, 0f, scale * 19f, Array( 0f, 0.25f, 1f ),
      ...
      private final class ButtonStripImpl( val iconTypes: Seq[ Type ], scheme: ColorScheme )
      extends Box( BoxLayout.X_AXIS ) with ButtonStrip {
         private val (buttonSeq, buttonMap, typeMap) = {
            var bMap = Map.empty[ Type, AbstractButton ]
            var tMap = Map.empty[ AbstractButton, Type ]
            var sq   = IIdxSeq.empty[ AbstractButton ]
            val it = iconTypes.iterator
            if( it.hasNext ) {
               val n1   = it.next()
               val pos1 = if( it.hasNext ) segmentFirst else segmentOnly
               val b1   = makeButton( pos1, n1, scheme )
               bMap += n1 -> b1
               tMap += b1 -> n1
               sq :+= b1
               while( it.hasNext ) {
         ...
         def button( iconType: Type ) : Option[ AbstractButton ] = buttonMap.get( iconType )
         def iconType( button: AbstractButton ) : Option[ Type ] = typeMap.get( button )

         private def makeButton( pos: String = "only", shape: Type, scheme: ColorScheme ) : JButton = {
   ...
   //         b.setPreferredSize( new Dimension( 50, 50 ))
      ...
      def makeButtonStrip( types: Seq[ Type ], scheme: ColorScheme = DarkScheme ) : JComponent with ButtonStrip =
         new ButtonStripImpl( types, scheme )
      ...
      trait ButtonStrip {
         ...
         def button( iconType: Type ) : Option[ AbstractButton ]
         def iconTypes: Seq[ Type ]
         def iconType( button: AbstractButton ) : Option[ Type ]
      }
